...
class PredictionBuilder:
    """
    Constructs Morphing Model for the wilson coefficient of each operator
    """
    
    def __init__(self, nOps: int, samples: List[float], preds: List[List[float]]):
        """Constuctor for PredictionBuilder"""
        
        self.nOps = nOps
        self.nSamples = int((nOps + 1) * (nOps + 2) / 2)
        self.model = self.initRM(nOps, samples, preds)
        
    def initRM(self, nOps: int, samples: List[float], preds: List[List[float]]) -> LinearRegression:
    ...
    def makeCoeffPoint(self, ci):
        ...
        X = X.reshape(1, triangularNumber(len(ci)))
        return X
